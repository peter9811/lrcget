...
function romanizeChinese(text, isSimple) {
  const pinyinArray = pinyin(text, {
    style: isSimple ? pinyin.STYLE_NORMAL : pinyin.STYLE_TONE,
  });
...
function getRomanizedText(text, language, isSimple) {
  if (language === "ja") {
    return romanizeJapanese(text);
  } else if (language === "zh") {
    return romanizeChinese(text, isSimple);
  } else {
    return text;
  }
}
